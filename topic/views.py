...
def topics(request, username=None):
    ...
    if request.method == 'POST':
        ...
        if topic_id:
            ...
            result = get_topic_detail(request, is_me, topic_id, user, topic)
            return JsonResponse(result)

        # 带查询字符串 /v1/topics/clark?category=tec
        category = request.GET.get('category')
        if category in ['tec', 'no-tec']:
            if is_me:
                topics_list = models.Topic.objects.filter(author=user, category=category)
            else:
                topics_list = models.Topic.objects.filter(author=user, limit='public', category=category)
        else:
            if is_me:
                topics_list = models.Topic.objects.filter(author=user)
            else:
                topics_list = models.Topic.objects.filter(author=user, limit='public')

        result = make_topics_res(user, topics_list)
        return JsonResponse(result)
...
def make_topics_res(author, author_topics):
    page_dic = paginator_dic(author_topics, 5, 1)
    print(page_dic)
    data = []
    for item in page_dic.get('list'):
        ...
        data.append(obj)
    result = {"code": 200, 'data': {'nickname': author.nickname, "topics": data}}
    return result


def get_topic_detail(request, is_me, topic_id, user, topic):
    next_id = None
    next_title = None
    last_id = None
    last_title = None
    ...
    for item in messages:
        message_count += 1
        replys = []
        for reply in item.reply_set.all().order_by('-created_time'):
            message_count += 1
            reply_to = None
            if reply.reply_to:
